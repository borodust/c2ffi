...
namespace c2ffi {
    ...
    public:
        explicit Decl(std::string name) : _name(std::move(name)), _id(0) {}

        explicit Decl(clang::NamedDecl *d);
    ...
    public:
        UnhandledDecl(std::string name, std::string kind)
                : Decl(std::move(name)), _kind(std::move(kind)) {}
    ...
    public:
        TypeDecl(std::string name, Type *type)
                : Decl(std::move(name)), _type(type) {}

        ~TypeDecl() override { delete _type; }
    ...
    public:
        VarDecl(std::string name, Type *type, std::string value = "",
                ...
                : TypeDecl(std::move(name), type), _value(std::move(value)), _is_extern(is_extern),
    ...
    public:
        explicit RecordDecl(std::string name, bool is_union = false)
                : Decl(std::move(name)), _is_union(is_union), _bit_size(0),
                  _bit_alignment(0) {}
    ...
    class EnumDecl : public Decl {
        NameNumVector _v;
    public:
        explicit EnumDecl(std::string name) : Decl(std::move(name)) {}
    ...
    public:
        ...
        struct ParentRecord {
            ...
            ParentRecord(std::string n, Access ac, int64_t off,
                         bool virt)
                    : name(std::move(n)), access(ac), parent_offset(off),
                      is_virtual(virt) {}
        };
    ...
    public:
        ...
        CXXRecordDecl(C2FFIASTConsumer *ast,
                      ...
                      bool is_class = false,
                      const clang::TemplateArgumentList *arglist = nullptr)
                : RecordDecl(std::move(name), is_union), _is_class(is_class),
    ...
    public:
        CXXFunctionDecl(C2FFIASTConsumer *ast,
                        ...
                        bool is_inline, clang::StorageClass storage_class,
                        const clang::TemplateArgumentList *arglist = nullptr)
                : FunctionDecl(ast, std::move(name), type, is_variadic, is_inline,
    ...
    public:
        explicit CXXNamespaceDecl(std::string name)
                : Decl(std::move(name)) {}
    ...
    public:
        ObjCInterfaceDecl(std::string name, std::string super,
                          bool is_forward)
                : Decl(std::move(name)), _super(std::move(super)), _is_forward(is_forward) {}
    ...
    public:
        ObjCCategoryDecl(Name name, Name category)
                : Decl(std::move(name)), _category(std::move(category)) {}
    ...
    public:
        explicit ObjCProtocolDecl(Name name)
                : Decl(std::move(name)) {}
